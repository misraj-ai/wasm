...
class Tree:
    ...
    def make_tree(self, selectolax_node, page_url, path_in_tree=[]):
        ...
        if tag in MEDIA_CONTAIN_INTERESTING_ATTRIBUTES_SET:
            caption = ""
            # Common HTML pattern: <figure> <img ...> <figcaption>Some text</figcaption> </figure>
            figcaption = selectolax_node.css_first("figcaption")
            if figcaption:
                caption = figcaption.text(separator=" ", strip=True)
            else:
                # Fallback: sometimes captions are in alt/title attributes or nearby text
                if selectolax_node.attrs.get("alt"):
                    caption = selectolax_node.attrs.get("alt", "")
                elif selectolax_node.attrs.get("title"):
                    caption = selectolax_node.attrs.get("title", "")
            return Node(
