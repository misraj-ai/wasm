...
class FilteringFunctions:
    ...
    def check_number_images(number_images, number_images_min_cutoff, number_images_max_cutoff):
        ...
        number_images_max_cutoff = np.inf if number_images_max_cutoff=='None' else number_images_max_cutoff
    ...
    def remove_diacritics(text):
        """
        Removes Arabic diacritics (harakat) from the text using regex.
        """
        # Regex to match Arabic diacritics (harakat)
        diacritics_regex = r'[\u064B-\u065F\u0670]'
        return re.sub(diacritics_regex, '', text)
    ...
    @staticmethod
    def compute_similarity_percentage(seq1, seq2):
        """Computes similarity percentage using fuzzy matching (Levenshtein ratio)."""
        return fuzz.ratio(seq1, seq2)
    
    @staticmethod 
    def deduplicate_strings(strings, threshold=80):
        """Identifies duplicate strings based on Needleman-Wunsch similarity percentage."""
        duplicates = set()

        for i in range(len(strings)):
            if strings[i] is None:
                continue
            for j in range(i + 1, len(strings)):  # Avoid self and redundant comparisons
                seq1, seq2 = strings[i], strings[j]
                if seq2 is None:
                    continue
                similarity = FilteringFunctions.compute_similarity_percentage(seq1, seq2)


                if similarity >= threshold:
                    duplicates.add(j)  # Consider seq2 a duplicate of seq1
    ...
    def check_number_words(text, strip_characters, number_words_min_cutoff, number_words_max_cutoff):
        ...
        number_words_max_cutoff = np.inf if number_words_max_cutoff == 'None' else number_words_max_cutoff
    ...
    def check_character_repetition_ratio(
        ...
        processed_text = FilteringFunctions.remove_diacritics(text)

        character_repetition_ratio = FilteringFunctions.compute_character_repetition_ratio(
            text=processed_text, character_repetition_length=character_repetition_length
        )
        # print(f"Character repetition ratio {character_repetition_ratio}")
    ...
    def check_word_repetition_ratio(
        ...
        # print(f"word repetition ratio {word_repetition_ratio}")
    ...
    def check_stopword_ratio(text, strip_characters, stopwords, stopword_ratio_min_cutoff):
        ...
        stopword_ratio = FilteringFunctions.compute_stopword_ratio(
            text=processed_text, strip_characters=strip_characters, stopwords=stopwords
        )
        # print(f"stopword ratio {stopword_ratio}")
    ...
    def check_flagged_word_ratio(
        ...
        # print(f"flagged word ratio {flagged_word_ratio}")
    ...
    def check_punctuation_ratio(
        ...
        # print(f"punctuation ratio {punctuation_ratio}")
    ...
    def check_common_word_ratio(
        ...
        # print(f"common word ratio {common_word_ratio}")
    ...
    def compute_lang_id_pred_score(text, lang_id_model):
        ...
        pred_lang_id = 'ar' if pred_lang_id == 'arz' else pred_lang_id
...
class WebDocumentFilteringNodeLevel:
    ...
    def __init__(
        ...
        #--------------------------------------------------------------------------
        # Loads a list of common words in English we will not be loading it as 
        # The arabic content is rare and it does not matter if it's AI-generated 
    ...
    def __call__(self, web_document):
            ...
            if image is not None:
                ...
                if self.cond_check_format:
                    if not FilteringFunctions.check_format(image_metadata=meta, valid_formats=self.valid_formats):
                        # print("INVALID FORMAT AT NODE LEVEL")
                ...
                if self.cond_check_size_image:
                    if not FilteringFunctions.check_size_image(
                        ...
                        # print("INVALID SIZE AT NODE LEVEL")
                        indices_to_remove.add(ind)
                        continue
                ...
                for ind_par, paragraph in enumerate(paragraphs):
                    ...
                    if self.cond_check_number_words_node_level:
                        if not FilteringFunctions.check_number_words(
                            ...
                            # print("INVALID NUMBER OF WORDS AT NODE LEVEL")
                    ...
                    if self.cond_check_character_repetition_ratio_node_level:
                        if not FilteringFunctions.check_character_repetition_ratio(
                            ...
                            # print("EXCEEDED CHARACTER REPETITION RATIO AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_word_repetition_ratio_node_level:
                        if not FilteringFunctions.check_word_repetition_ratio(
                            ...
                            # print("EXCEEDED CHARACTER WORD RATIO AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_special_character_ratio_node_level:
                        if not FilteringFunctions.check_special_character_ratio(
                            ...
                            # print("EXCEEDED SPECIAL CHARACTER RATIO AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_stopword_ratio_node_level:
                        if not FilteringFunctions.check_stopword_ratio(
                            ...
                            # print("HIGH STOPWORD RATIO AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_flagged_word_ratio_node_level:
                        if not FilteringFunctions.check_flagged_word_ratio(
                            ...
                            # print("FLAGGED WORDS AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_punctuation_ratio_node_level:
                        if not FilteringFunctions.check_punctuation_ratio(
                            ...
                            # print("HIGH PUNCTUATION RATIO AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
                    ...
                    if self.cond_check_lang_id_node_level:
                        if not FilteringFunctions.check_lang_id(
                            ...
                            # print("NON-ARABIC TEXT AT NODE LEVEL")
                            paragraphs_indices_to_remove.add(ind_par)
                            continue
        ...
        web_document["images_captions"] = [el for ind, el in enumerate(web_document["images_captions"]) if ind not in indices_to_remove]
...
class WebDocumentDocLevelDeDup:
  def __init__(
    self,
    cond_deduplicate_nodes_doc_level,
    deduplication_max_cuttoff,
  ):
    self.cond_deduplicate_nodes_doc_level = cond_deduplicate_nodes_doc_level
    self.deduplication_max_cuttoff = deduplication_max_cuttoff
  
  def __call__(self,web_document):
    texts = web_document["texts"]
    images = web_document["images"]
    images_captions = web_document["images_captions"]
    if self.cond_deduplicate_nodes_doc_level:
            duplicated_texts = FilteringFunctions.deduplicate_strings(texts,self.deduplication_max_cuttoff)
            texts = [text for i,text in enumerate(texts) if i not in duplicated_texts]
            web_document["texts"] = texts
            web_document["images"] = [imgae for i,imgae in enumerate(images) if i not in duplicated_texts]
            web_document["images_captions"] = [el for i,el in enumerate(images_captions) if i not in duplicated_texts]
    return web_document
  def __reduce__(self):
        return (
            self.__class__,
            (
              self.cond_deduplicate_nodes_doc_level,
              self.deduplication_max_cuttoff
                ),
        )
    ...
    def __call__(self, web_document):
        ...
        if self.cond_check_number_images:
            if not FilteringFunctions.check_number_images(
                ...
                # print("HIGH NUMBER OF IMAGES AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_number_words_doc_level:
            if not FilteringFunctions.check_number_words(
                ...
                # print("UNSUITABLE WORD COUNT AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_character_repetition_ratio_doc_level:
            if not FilteringFunctions.check_character_repetition_ratio(
                ...
                # print("HIGH CHARACTER REPETITION RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_word_repetition_ratio_doc_level:
            if not FilteringFunctions.check_word_repetition_ratio(
                ...
                # print("HIGH WORD REPETITION RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_special_character_ratio_doc_level:
            if not FilteringFunctions.check_special_character_ratio(
                ...
                # print("HIGH SPECIAL CHARACTER RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_stopword_ratio_doc_level:
            if not FilteringFunctions.check_stopword_ratio(
                ...
                # print("HIGH STOPWORD RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_flagged_word_ratio_doc_level:
            if not FilteringFunctions.check_flagged_word_ratio(
                ...
                # print("HIGH FLAGGED WORD RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_punctuation_ratio_doc_level:
            if not FilteringFunctions.check_punctuation_ratio(
                ...
                # print("HIGH PUNCTUATION RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_common_word_ratio_doc_level:
            if not FilteringFunctions.check_common_word_ratio(
                ...
                # print("HIGH COMMON WORD RATIO AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_lang_id_doc_level:
            if not FilteringFunctions.check_lang_id(
                ...
                # print("NON ARABIC TEXT AT DOCUMENT LEVEL")
                return False
        ...
        if self.cond_check_perplexity_score_doc_level:
            if not FilteringFunctions.check_perplexity_score(
                ...
                # print("HIGH PERPLEXITY SCORE AT DOCUMENT LEVEL")
                return False
