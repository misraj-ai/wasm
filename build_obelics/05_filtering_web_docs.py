...
def download_model(url:str,save_path:str)->None:
    ...
    Args:
        ...
        save_path: the path of the file (should be in the same format as the provided data)
    """

    response = requests.get(url)
    if response.status_code==200:
        print(f"[INFO] successfully downloaded the data saving it to ... {save_path}")
        with open(save_path,"wb") as f :
            f.write(response.content)

    else:
        ValueError("Invalid URL please check it again")
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ar.sp.model",#Changed to the same directory this is just for testing and can be changed later
        help="The path of the SentencePiece model.",
    )
    parser.add_argument(
        "--path_kenlm_model",
        type=str,
        default="ar.arpa.bin", #Changed to the same directory this is just for testing and can be changed later
        help="The path of the KenLM model.",
    )
    #-----------------------------------------------------------------------
    #Added this argument to download The sentencepiece and KenLM models 
    # --------------------------------------------------------------------- 
    parser.add_argument(
        "--download",
        type=bool,
        default=True,
        help="Download the sentencepiece and KenLM models"
    )
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------------------------
    if args.download: 
        download_model("https://huggingface.co/edugp/kenlm/resolve/main/wikipedia/ar.arpa.bin?download=true",
                 args.path_kenlm_model) #KenLM model
        download_model("https://huggingface.co/edugp/kenlm/resolve/main/wikipedia/ar.sp.model?download=true",
                 args.spath_sentencepiece_model)# Sentencepiece model
        download_model("https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin",
                 args.path_lang_id_model)
